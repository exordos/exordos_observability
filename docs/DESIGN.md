# Exordos Observability Element — Design
...
└── .github/{workflows/{tests.yaml, build.yml}, scripts/wait-for-element.sh}
```
...
The workflow has two jobs, mirroring `exordos_mail` / `exordos_s3` / `exordos_db`. **`Build`** runs on the `[self-hosted, vm]` runners (packer, KVM and the exordos CLI are preinstalled there), builds the element and pushes it to repo.exordos.com (`exordos_repo` target, `PUSH_EXORDOS_CFG` secret). **`E2E`** bootstraps a throwaway core on a GitHub runner, installs `metapaas` and then `victoriaaas`, `grafanaaas`, `observability` at the version `Build` pushed, waits for all of them to be ACTIVE (`.github/scripts/wait-for-element.sh`), and runs the functional suite. Note that the push happens before the E2E run, so a pushed build is not proof the E2E passed. Two things are easy to break and not derivable from the code:

- **`exordos bootstrap … --pool-agent-placement local` is mandatory on a GitHub runner.** Since exordos CLI PR #350 (2026-08-26) the default is `core`, which runs the pool agent inside the core VM and expects libvirt over `qemu+tcp://10.20.0.1`. `exordos compute hypervisors init` on the runner only sets up the local socket, so with the default nothing ever creates compute nodes and every element sits in `IN_PROGRESS` — the symptom of PR #1's first CI run. `exordos_s3` and `exordos_mail` CI hit the same thing.

- **The chain takes 20+ minutes on a runner.** Measured on the 2026-09-15 run: the metapaas CP image is pulled from repo.exordos.com and net-booted into its VM (~18 min), only then does the CP create the victoria/grafana DP nodes, and those VMs need a few more minutes to boot. The wait step therefore allows 45 min per element. Don't answer a wait timeout by uninstalling/reinstalling: that throws away all of that progress.

Future candidates (not started, no code): AlertManager plugin (third building block, same pattern as Grafana), VictoriaMetrics/VictoriaLogs HA (`kind: distributed`, multiple `NodeSet`s per instance — no existing plugin does this yet, will need new design work). vmauth JWT/IAM integration (Variant I/III in [`docs/DESIGN-victoria-auth.md`](DESIGN-victoria-auth.md)) — currently Basic Auth is used.
