...
The workflow bootstraps a throwaway core on a GitHub runner and installs the freshly built `observability` from the local build output with a single `exordos deploy --element observability --timeout 1800 --force` (local-realm mode: the CLI serves `output/` in-process, registers it as a dev repository and installs from it; metapaas/victoriaaas/grafanaaas come in as dependencies). Two things there are easy to break and not derivable from the code:

- **`exordos bootstrap … --pool-agent-placement local` is mandatory on a runner.** Since exordos CLI PR #350 (2026-08-26) the default is `core`, which runs the pool agent inside the core VM and expects libvirt over `qemu+tcp://10.20.0.1`. `exordos compute hypervisors init` on the runner only sets up the local socket, so with the default nothing ever creates compute nodes and every element sits in `IN_PROGRESS` until the deploy times out — the symptom of PR #1's first CI run. `exordos_s3` and `exordos_mail` CI hit the same thing.

- **The whole chain takes 20+ minutes on a runner, and a deploy timeout must not be answered by `deploy --force` again.** Measured on the 2026-09-15 run: the metapaas CP image is pulled from repo.exordos.com and net-booted into its VM (~18 min), only then does the CP create the victoria/grafana DP nodes, and those VMs need a few more minutes to boot. `exordos deploy --force` uninstalls and re-creates the element, so retrying it after a timeout throws away all of that progress. The workflow uses `--timeout 1800` and, if the element is installed and still reconciling when the CLI gives up, keeps polling `ee l` for ACTIVE instead of re-deploying.
