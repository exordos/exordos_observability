...
Commands: `tox -e py312` (unit tests), `tox -e ruff-check` (lint), `tox -e mypy` (types), `make build` (build DP images + manifests, needs the `exordos` CLI + packer + KVM). Functional tests need a live stand with the `observability` element deployed (`exordos deploy --element observability`; see `.github/workflows/build.yml`).
