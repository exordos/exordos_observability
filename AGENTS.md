...
- Functional tests will `pytest.skip()` gracefully if no live stand is reachable — they are not meant to run in a plain CI checkout without the `observability` element having been deployed first.
