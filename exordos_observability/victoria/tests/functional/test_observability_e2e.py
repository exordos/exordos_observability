...
class TestVictoriaStorages:
    def test_metrics_roundtrip(self, victoria_metrics_endpoint, vmauth_auth) -> None:
    ...
    def test_logs_roundtrip(self, victoria_logs_endpoint, vmauth_auth) -> None:
        ...
        while time.monotonic() < deadline:
            lines = ft.query_logs(
                victoria_logs_endpoint,
                f'job:exordos-observability-e2e AND "{marker}"',
                auth=vmauth_auth,
            )
            if lines:
                break
