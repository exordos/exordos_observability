#!/usr/bin/env python3
"""Prepare an Exordos Core environment for exordos_observability integration testing.

Steps:
  1. Generate SSH key pair.
  2. Build exordos_observability (victoriaaas + grafanaaas DP images + manifests,
     from one exordos.yaml) + Python wheel (from --project-dir).
     Optionally build exordos_metapaas CP image (from --metapaas-dir).
  3. Serve artifacts via a local HTTP server when requested.
  4. Install metapaas element (from official repo, or local if --metapaas-dir given);
     wait for CP node ACTIVE.
  5. Deploy observability from the local build output, or install it from the
     repository when the HTTP server mode is enabled.
...
Usage::
    ...
    python prepare_env.py \\
        --project-dir . \\
        --output-dir /tmp/metapaas-observability-build \\
...
# exordos.yaml declares these two dataplane-image elements from one project
OBSERVABILITY_ELEMENTS = ["victoriaaas", "grafanaaas"]
...
def _build_wheel(project_dir: str, output_dir: str) -> pathlib.Path:
    """Build Python wheel for exordos_observability."""
    ...
    wheels = list(dist_dir.glob("exordos_observability-*.whl"))
    if not wheels:
...
def _publish_to_serve_dir(
    ...
    observability_output: pathlib.Path,
    ...
    victoriaaas/grafanaaas DP images + manifests: serve_root/<element>/<version>/...
    pip wheel:           serve_root/simple/exordos_observability-*.whl
    """
    ...
    # exordos build with multiple manifest elements lays images/manifests out
    # per-element already under observability_output; just fan them out by
    # element name into the serve root.
    obs_ver = _read_version(observability_output)
    for element in OBSERVABILITY_ELEMENTS:
        img_dir = serve_root / element / obs_ver / "images"
        img_dir.mkdir(parents=True, exist_ok=True)
        for img in (observability_output / "images").glob(
            f"*{element.replace('aas', '')}*.zst"
        ):
            dst = img_dir / img.name
            if not dst.exists():
                shutil.copy2(img, dst)
            _log(f"  {element} DP image: {element}/{obs_ver}/images/{img.name}")
        for mf in (observability_output / "manifests").glob(f"{element}.yaml"):
            dst = serve_root / element / obs_ver / mf.name
            dst.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(mf, dst)
            _log(f"  {element} manifest: {element}/{obs_ver}/{mf.name}")
...
def _ee_deploy(
    ...
    for attempt in range(1, 7):
        try:
            ...
            if attempt == 5:
                raise
            _log(f"Deploy attempt {attempt}/5 failed; retrying in 10s")
            time.sleep(10)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Prepare Exordos Core for exordos_observability integration testing",
    ...
    p.add_argument(
        "--project-dir",
        default=".",
        help="Path to exordos_observability repository (default: .)",
    )
    p.add_argument("--output-dir", default="output", help="Directory for build output")
    ...
    p.add_argument(
        ...
        default="0.0.1",
        help="observability element version to install",
    )
    p.add_argument(
...
def main(argv: list[str] | None = None) -> None:
    ...
    observability_output = output_dir / "observability"
    ...
    if not args.skip_build:
        ...
        _log("Step 2b: Building exordos_observability (DP images + manifests)")
        obs_vars: dict[str, str] = {}
        if repository_url:
            obs_vars["repository"] = repository_url
        if index_url:
            obs_vars["index_url"] = index_url
        _build(args.project_dir, str(observability_output), pub_key, obs_vars)

        _log("Step 2c: Building Python wheel for exordos_observability")
    ...
    if not args.no_http_server:
        if wheel_path is None:
            wheels = list((wheel_output / "dist").glob("exordos_observability-*.whl"))
            if not wheels:
        ...
        http_proc = _start_http_server(str(serve_root), args.http_port)
    ...
    if args.no_http_server:
        _log("Step 5: Deploying observability element from local build output")
        _ee_deploy(
            "observability",
            args.endpoint,
            args.username,
        ...
        _log("Step 5: Installing observability element from repository")
        _ee_install(
            "observability",
        ...
        _log("Step 5a: Waiting for observability element ACTIVE")
        _wait_for_element(
            "observability",
            "ACTIVE",
            args.endpoint,
    ...
    _print_summary(args, cp_ip)


def _print_summary(args, cp_ip) -> None:
    _log("=" * 60)
    _log("Environment ready! Suggested env vars for functional tests:")
    _log("")
    # Print only variable names, not values — secrets must not leak into
    # CI logs or terminal transcripts. The caller already has the values
    # in their environment; functional tests read them from os.environ.
    print("  export EXORDOS_ENDPOINT=<your-core-endpoint>", flush=True)
    print("  export EXORDOS_USERNAME=<your-core-username>", flush=True)
    print("  export EXORDOS_PASSWORD=<your-core-password>", flush=True)
    print(f"  export EXORDOS_OBSERVABILITY_CP_URL=http://{cp_ip}:8080", flush=True)
