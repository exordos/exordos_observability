...
SECRET_PASSWORDS = "/v1/secret/passwords/"

# vmauth Basic Auth credentials for read requests through vmauth.
# The username is set by the composition manifest; the password is fetched
# from the Core Secret Manager at session scope (see ``vmauth_auth``).
VMAUTH_USERNAME = os.environ.get("EXORDOS_VMAUTH_USERNAME", "grafana-reader")
VMAUTH_PASSWORD_NAME = os.environ.get(
    "EXORDOS_VMAUTH_PASSWORD_NAME", "vmauth_reader_password"
)
...
@pytest.fixture(scope="session")
def vmauth_auth(core_client) -> tuple[str, str]:
    """Fetch the vmauth grafana-reader credentials from the Secret Manager.

    vmauth protects read paths (``/api/v1/query``, ``/select/*``) with
    Basic Auth. The password is generated by the composition manifest and
    stored in the Core Secret Manager; the ``vmauth`` field on the Victoria
    instance is HIDDEN in the API, so the secret must be fetched directly.
    """
    passwords = core_client.filter(SECRET_PASSWORDS, name=VMAUTH_PASSWORD_NAME)
    if not passwords:
        pytest.skip(
            f"No secret password named '{VMAUTH_PASSWORD_NAME}' found — is "
            f"the observability composition manifest deployed?"
        )
    password = passwords[0].get("value")
    if not password:
        pytest.skip(f"Secret password '{VMAUTH_PASSWORD_NAME}' has no value yet")
    return (VMAUTH_USERNAME, password)
...
def query_metric(
    metrics_endpoint: str, query: str, auth: tuple[str, str] | None = None
) -> dict:
    resp = requests.get(
...
def query_logs(
    logs_endpoint: str, query: str, auth: tuple[str, str] | None = None
) -> list:
    resp = requests.get(
        f"{logs_endpoint}/select/logsql/query",
        params={"query": query},
        auth=auth,
        timeout=30,
    )
