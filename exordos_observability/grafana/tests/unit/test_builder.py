...
class TestRefreshDashboardContent:
    ...
    def test_non_object_json_marks_error_and_continues(self, b, monkeypatch) -> None:
        ...
        monkeypatch.setattr(source, "_fetch_dashboard", lambda core_client=None: [])
    ...
    def test_non_object_json_does_not_abort_other_dashboards(
        ...
        monkeypatch.setattr(bad_source, "_fetch_dashboard", lambda core_client=None: [])
...
class TestRenderNodeConfig:
    ...
    def test_oidc_auth_renders_oauth_vars(self, infra_builder) -> None:
        ...
        # The admin password is NOT delivered via the env file for OIDC
        # deployments. GF_SECURITY_ADMIN_PASSWORD only applies on first
        # admin-user creation; deriving it from client_secret would
        # break the reload API (401) when the secret is rotated. Instead,
        # the DP bootstrap script generates a stable password durably.
        assert "GF_SECURITY_ADMIN_PASSWORD" not in env

    def test_oidc_env_is_stable_across_client_secret_rotation(
        self, infra_builder
    ) -> None:
        """Rotating the OIDC client_secret must not change the admin
        password line (there is none), so the reload API credentials
        remain stable."""
        inst1 = self._make_instance(
            auth=auth_kinds.OidcAuth(
                client_id="grafana-oidc",
        ...
        inst2 = self._make_instance(
            auth=auth_kinds.OidcAuth(
                client_id="grafana-oidc",
                client_secret="secret-v2-rotated",
                auth_url="http://core.local/auth",
                token_url="http://core.local/token",
                api_url="http://core.local/userinfo",
                root_url="http://observability.local.genesis-core.tech",
        ...
        env1 = self._env(infra_builder, inst1)
        env2 = self._env(infra_builder, inst2)
