...
class GrafanaInstance(models.GrafanaInstance, ua_models.InstanceWithDerivativesMixin):
    ...
    def _render_node_configs(self) -> tp.Dict[str, str]:
        ...
        if isinstance(self.auth, auth_kinds.PasswordAuth):
            ...
            a = self.auth
            # The admin password for OIDC-only deployments is NOT set
            # here. GF_SECURITY_ADMIN_PASSWORD only applies on first
            # admin-user creation; if it were derived from the OIDC
            # client_secret, rotating the secret would change the env
            # var but leave the DB password stale — causing the reload
            # API to return 401. Instead, the DP bootstrap script
            # generates a random password once and stores it durably
            # in GRAFANA_ADMIN_PASSWORD_FILE (see grafana_dp_bootstrap.sh).
            auth_config = (
